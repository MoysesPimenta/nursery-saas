...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  return requirePermission('manage_employees', async (req: NextRequest, user) => {
    try {
      ...
      if (error) {
        if (error.code === '23505') return errorResponse('This allergy is already linked to this employee', 409);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  return requirePermission('manage_employees', async (req: NextRequest, user) => {
    try {
      ...
      if (!allergyId || !validateUUID(allergyId)) return errorResponse('Invalid or missing allergy_id parameter', 400);
      ...
      if (error) {
        if (error.code === 'PGRST116') return errorResponse('Allergy not found for this employee', 404);
        return errorResponse(error.message, 400);
      }

      return successResponse({ message: 'Allergy removed from employee', id: data.id });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Internal server error';
      return errorResponse(message, 500);
    }
